%
% Режим слабого торможения при alpha > 0
%
function [is, uMat, xMat, timeVec, value] = weakAlpha(T, L, S, eps, alpha, k1, k2, N, modeStr)
    ...
    for xi = S-eps : xiStep : S+eps
        for t1 = tStep : tStep : T-tStep
            ...
            x11 = L - x21.*(1 - exp((k1+1).*(t1 - T)))./(k1+1);
            if x11 < 0 || x21 < 0
                continue;
            end
            % Найдем A и B
            ...
            t2 = log(-B / A) / (k1+1);
            if A < 0 && (A + B) > alpha/2 && t2 < T && ...
                        abs((A*exp((k1+1)*t1) + B) - alpha/2) < 0.001
                is = 1;
                timeVecBefore = 0:tStep:t1;
                timeVecAfter  = t1:tStep:T;
                psi2VecBefore = A .* exp((k1+1).*timeVecBefore) + B;
                u1Vec = [(psi2VecBefore - alpha/2), zeros(1, numel(timeVecAfter))];
                u2Vec = k1 .* ones(1, numel(u1Vec));
                x1Vec   = 1 / (k1 + 1) .* ...
                        (-A./(k1+1) + A./(k1+1).* cosh((k1+1).*timeVecBefore) + ...
                        (B - alpha/2)    .* (timeVecBefore - 1/(k1+1)*(1 - exp(-(k1+1).*timeVecBefore))));
                x2Vec   = 1 / (k1 + 1) .* ...
                        (A           .* sinh((k1+1).*timeVecBefore) + ...
                        (B - alpha/2)    .* (1 - exp(-(k1+1).*timeVecBefore)));
                x1Vec = [x1Vec, x11 + x21 / (k1 + 1) .* (1 - exp((k1+1).*(t1-timeVecAfter)))];
                x2Vec = [x2Vec, x21.*exp((k1+1).*(t1-timeVecAfter))];    
                % Нарисуем
                if strcmp(modeStr, 'draw')
                    plot(x1Vec, x2Vec, 'g');
                end
                % Проверим на наилучшесть
                curValue = trapz([timeVecBefore, timeVecAfter], u1Vec.^2 + alpha.*abs(u1Vec));
                if curValue < value
                    uMat = [u1Vec', u2Vec'];
                    xMat = [x1Vec', x2Vec'];

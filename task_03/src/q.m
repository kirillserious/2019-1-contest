% Функция-копия reachset, которая в режиме 'draw' рисует разные траетории
% разными цветами и обозначает неподвижные точки
%
% Arguments
...
function [xes, switches] = q(alpha, t, mode)
    ...
    if strcmp(mode, 'draw')
        ...
        p2 = plot([NaN, NaN]);
        ax.ColorOrderIndex = 1;
        xxx = fzero(@(x) alpha - x - 3 * sin(3*(x^3)), 0);
        p3 = plot(xxx, 0, '*');
        ax.ColorOrderIndex = 2;
        xxx = fzero(@(x) -alpha - x - 3 * sin(3*(x^3)), 0);
        p4 = plot(xxx, 0, '*');
        legend([p1, p2, p3, p4], {'Траектории при +\alpha', 'Траектории при -\alpha', 'Неподвижная точка при +\alpha', 'Неподвижная точка при -\alpha'});
        hold off;
    end
end
